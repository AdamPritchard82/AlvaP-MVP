...
function createSQLiteTables() {
  ...
  tables.forEach((tableSQL, index) => {
    try {
      console.log(`Creating SQLite table ${index + 1}...`);
      
      db.run(tableSQL, (err) => {
        if (err) {
          console.error(`Error creating SQLite table ${index + 1}:`, err);
          console.error('SQL:', tableSQL);
        } else {
          console.log(`✅ SQLite table ${index + 1} created successfully`);
        }
        ...
        if (completedTables === tables.length) {
          console.log('All SQLite tables created, seeding data...');
          ...
          setTimeout(() => {
            try {
              ...
              console.error('Error seeding SQLite data:', err);
            }
          }, 1000);
        }
      });
      ...
      if (completedTables === tables.length) {
        console.log('All SQLite tables processed, seeding pipeline stages...');
        setTimeout(() => {
          try {
            seedDefaultPipelineStages();
          } catch (err) {
            console.error('Error seeding SQLite pipeline stages:', err);
          }
        }, 100);
      }
    }
...
function createPostgreSQLTables() {
  ...
  createPostgreSQLTablesSequentially(tables, 0);
}

async function createPostgreSQLTablesSequentially(tables, index) {
  if (index >= tables.length) {
    console.log('All PostgreSQL tables created, seeding data...');
    await seedPostgreSQLData();
    return;
  }

  try {
    console.log(`Creating PostgreSQL table ${index + 1}/${tables.length}...`);
    await db.query(tables[index]);
    console.log(`✅ PostgreSQL table ${index + 1} created successfully`);
    
    // Create next table
    setTimeout(() => createPostgreSQLTablesSequentially(tables, index + 1), 100);
  } catch (err) {
    console.error(`❌ Error creating PostgreSQL table ${index + 1}:`, err);
...
async function seedPostgreSQLData() {
  try {
    ...
    await seedPostgreSQLPipelineStages();
    
    // Seed plans
    await seedPostgreSQLPlans();
...
async function seedPostgreSQLPipelineStages() {
  const defaultStages = [
  ...
  for (const stage of defaultStages) {
    try {
      ...
      console.log(`✅ Seeded PostgreSQL pipeline stage: ${stage.name}`);
    } catch (err) {
      console.error(`❌ Error seeding PostgreSQL pipeline stage ${stage.name}:`, err);
    }
  }
}

async function seedPostgreSQLPlans() {
  const defaultPlans = [
    {
      id: 'free',
      name: 'Free',
  ...
  for (const plan of defaultPlans) {
    try {
      ...
      console.log(`✅ Seeded PostgreSQL plan: ${plan.name}`);
    } catch (err) {
      console.error(`❌ Error seeding PostgreSQL plan ${plan.name}:`, err);
    }
  }
}
